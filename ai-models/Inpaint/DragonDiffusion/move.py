...
def move(img,mask,x_s,y_s,x_e,y_e):#@cherry pass mask as numpy 
...
if __name__=="__main__":
    img=cv2.imread("orginalimg_from_get_move.png")
    

    mask=cv2.imread("run_move_mask.png")
    s_t=time.time()
    img_out=move(img,mask,250,250,300,300)
    print(time.time()-s_t)
